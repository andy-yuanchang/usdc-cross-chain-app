...
import { WalletConnectionError, WalletSwitchChainError } from '@/constants/errors';
...
export default function useWallet() {
  ...
  const [connectionStatus, setConnectionStatus] = useState<WalletConnectionStatus>('unconnected')
  const [transactionStatus, setTransactionStatus] = useState<TransactionStatus | null>(null);

  const connect = async (name: string) => {
    const wallet = getWallet(name)
    if (!wallet) {
      setError(new Error('No wallet detected, please choose a wallet to connect'))
      return
    }
    try {
      setError(null)
  ...
  return { isConnected: connectionStatus === 'connected', cancel: () => setConnectionStatus('unconnected'), connectionStatus, connect, switchChain, transactionStatus, error };
}